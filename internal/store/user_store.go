...
func (s *UserStore) GetByUsername(ctx context.Context, username string) (*models.User, error) {
	...
	err := s.pool.QueryRow(ctx,
		`SELECT id, username, email, password_hash, role, active, company_id, default_truck_id, first_name, last_name, created_at, updated_at
		 FROM users WHERE username = $1`, username,
	).Scan(&u.ID, &u.Username, &u.Email, &u.PasswordHash, &u.Role, &u.Active, &u.CompanyID, &u.DefaultTruckID, &u.FirstName, &u.LastName, &u.CreatedAt, &u.UpdatedAt)
	if err != nil {
...
func (s *UserStore) ListByCompany(ctx context.Context, companyID int) ([]models.User, error) {
	rows, err := s.pool.Query(ctx,
		`SELECT id, username, email, role, active, company_id, default_truck_id, first_name, last_name, created_at, updated_at
	...
	users, err := pgx.CollectRows(rows, func(row pgx.CollectableRow) (models.User, error) {
		var u models.User
		if err := row.Scan(&u.ID, &u.Username, &u.Email, &u.Role, &u.Active, &u.CompanyID, &u.DefaultTruckID, &u.FirstName, &u.LastName, &u.CreatedAt, &u.UpdatedAt); err != nil {
...
func (s *UserStore) Create(ctx context.Context, u *models.User) error {
	err := s.pool.QueryRow(ctx,
		`INSERT INTO users (username, email, password_hash, role, active, company_id, default_truck_id, first_name, last_name)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		 RETURNING id, created_at, updated_at`,
		u.Username, u.Email, u.PasswordHash, u.Role, u.Active, u.CompanyID, u.DefaultTruckID, u.FirstName, u.LastName,

...
func (s *LookupStore) List(ctx context.Context) ([]LookupItem, error) {
	col := s.codeColumn()
	query := fmt.Sprintf("SELECT id, %s, description FROM %s ORDER BY %s", col, s.tableName, col)
...
func (s *LookupStore) GetByID(ctx context.Context, id int) (*LookupItem, error) {
	col := s.codeColumn()
	query := fmt.Sprintf("SELECT id, %s, description FROM %s WHERE id = $1", col, s.tableName)
	var item LookupItem
	if err := s.pool.QueryRow(ctx, query, id).Scan(&item.ID, &item.Code, &item.Description); err != nil {
...
func (s *LookupStore) Create(ctx context.Context, code, description string) (*LookupItem, error) {
	col := s.codeColumn()
	query := fmt.Sprintf("INSERT INTO %s (%s, description) VALUES ($1, $2) RETURNING id", s.tableName, col)
	var item LookupItem
	item.Code = code
	item.Description = description
	if err := s.pool.QueryRow(ctx, query, code, description).Scan(&item.ID); err != nil {
...
func (s *LookupStore) Update(ctx context.Context, id int, code, description string) error {
	col := s.codeColumn()
	query := fmt.Sprintf("UPDATE %s SET %s = $1, description = $2 WHERE id = $3", s.tableName, col)
	_, err := s.pool.Exec(ctx, query, code, description, id)
	if err != nil {

...
const damageColumns = `id, order_id, vehicle_id, trip_id, vin,
...
func scanDamage(row interface{ Scan(dest ...any) error }) (*models.VehicleDamage, error) {
	...
	err := row.Scan(
		&d.ID, &d.OrderID, &d.VehicleID, &d.TripID, &d.VIN,
...
func (s *DamageStore) ListByVehicle(ctx context.Context, vehicleID int) ([]models.VehicleDamage, error) {
	query := fmt.Sprintf("SELECT %s FROM vehicle_damage WHERE vehicle_id = $1 ORDER BY id", damageColumns)
	rows, err := s.pool.Query(ctx, query, vehicleID)
	if err != nil {
...
func (s *DamageStore) GetByID(ctx context.Context, id int) (*models.VehicleDamage, error) {
	query := fmt.Sprintf("SELECT %s FROM vehicle_damage WHERE id = $1", damageColumns)
	d, err := scanDamage(s.pool.QueryRow(ctx, query, id))
	if err != nil {
...
func (s *DamageStore) Create(ctx context.Context, d *models.VehicleDamage) error {
	err := s.pool.QueryRow(ctx,
		`INSERT INTO vehicle_damage (
			order_id, vehicle_id, trip_id, vin,
			damage_area, damage_type, damage_severity, description,
			inspection_point, inspected_by, inspection_date,
			claim_amount, claim_status
...
func (s *DamageStore) Delete(ctx context.Context, id int) error {
	_, err := s.pool.Exec(ctx, "DELETE FROM vehicle_damage WHERE id = $1", id)
	if err != nil {
...
const noteColumns = `id, vehicle_id, note_date, description, comment, created_by,
	created_at, updated_at`

func scanNote(row interface{ Scan(dest ...any) error }) (*models.VehicleNote, error) {
	var n models.VehicleNote
	err := row.Scan(
		&n.ID, &n.VehicleID, &n.NoteDate, &n.Description, &n.Comment, &n.CreatedBy,
...
func (s *NoteStore) ListByVehicle(ctx context.Context, vehicleID int) ([]models.VehicleNote, error) {
	query := fmt.Sprintf("SELECT %s FROM vehicle_notes WHERE vehicle_id = $1 ORDER BY note_date DESC, id DESC", noteColumns)
	rows, err := s.pool.Query(ctx, query, vehicleID)
	if err != nil {
...
func (s *NoteStore) Create(ctx context.Context, n *models.VehicleNote) error {
	err := s.pool.QueryRow(ctx,
		`INSERT INTO vehicle_notes (vehicle_id, note_date, description, comment, created_by)
		VALUES ($1,$2,$3,$4,$5)
		RETURNING id, created_at, updated_at`,
		n.VehicleID, n.NoteDate, n.Description, n.Comment, n.CreatedBy,
...
func (s *NoteStore) Delete(ctx context.Context, id int) error {
	_, err := s.pool.Exec(ctx, "DELETE FROM vehicle_notes WHERE id = $1", id)
	if err != nil {

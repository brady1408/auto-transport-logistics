...
func (s *DamageStore) Update(ctx context.Context, d *models.VehicleDamage) error {
	...
	if result.RowsAffected() == 0 {
		return fmt.Errorf("damage %d not found", d.ID)
	}
	return nil
}
...
func (s *NoteStore) ListByVehicle(ctx context.Context, vehicleID int) ([]models.VehicleNote, error) {
	...
	items, err := pgx.CollectRows(rows, func(row pgx.CollectableRow) (models.VehicleNote, error) {
		n, err := scanNote(row)
		if err != nil {
			return models.VehicleNote{}, err
		}
		return *n, nil
	})
	if err != nil {

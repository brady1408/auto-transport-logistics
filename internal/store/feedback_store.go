...
func (s *FeedbackStore) GetByID(ctx context.Context, id int) (*models.Feedback, error) {
	companyID := auth.GetCompanyID(ctx)
	query := fmt.Sprintf("SELECT %s FROM feedback f JOIN users u ON u.id = f.user_id WHERE f.id = $1 AND f.company_id = $2", feedbackColumns)
	fb, err := scanFeedback(s.pool.QueryRow(ctx, query, id, companyID))
	if err != nil {
...
func (s *FeedbackStore) Update(ctx context.Context, fb *models.Feedback) error {
	...
	_, err := s.pool.Exec(ctx,
		`UPDATE feedback SET status = $1, admin_notes = $2, updated_at = NOW() WHERE id = $3 AND company_id = $4`,
		fb.Status, fb.AdminNotes, fb.ID, companyID,
	)
	if err != nil {
...
func (s *FeedbackStore) Delete(ctx context.Context, id int) error {
	companyID := auth.GetCompanyID(ctx)
	_, err := s.pool.Exec(ctx, "DELETE FROM feedback WHERE id = $1 AND company_id = $2", id, companyID)
	if err != nil {

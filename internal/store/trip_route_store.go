...
const routeColumns = `id, trip_id, sequence, customer_id, customer_name, city, state,
...
func scanRoute(row interface{ Scan(dest ...any) error }) (*models.TripRoute, error) {
	...
	err := row.Scan(
		&r.ID, &r.TripID, &r.Sequence, &r.CustomerID, &r.CustomerName, &r.City, &r.State,
...
func (s *TripRouteStore) ListByTrip(ctx context.Context, tripID int) ([]models.TripRoute, error) {
	query := fmt.Sprintf("SELECT %s FROM trip_routes WHERE trip_id = $1 ORDER BY sequence, id", routeColumns)
...
func (s *TripRouteStore) GetByID(ctx context.Context, id int) (*models.TripRoute, error) {
	query := fmt.Sprintf("SELECT %s FROM trip_routes WHERE id = $1", routeColumns)
	r, err := scanRoute(s.pool.QueryRow(ctx, query, id))
	if err != nil {
...
func (s *TripRouteStore) Create(ctx context.Context, r *models.TripRoute) error {
	err := s.pool.QueryRow(ctx,
		`INSERT INTO trip_routes (trip_id, sequence, customer_id, customer_name, city, state, stop_type, miles, est_arrival)
		VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
		RETURNING id, created_at, updated_at`,
		r.TripID, r.Sequence, r.CustomerID, r.CustomerName, r.City, r.State, r.StopType, r.Miles, r.EstArrival,
...
func (s *TripRouteStore) Delete(ctx context.Context, id int) error {
	_, err := s.pool.Exec(ctx, "DELETE FROM trip_routes WHERE id = $1", id)
	if err != nil {

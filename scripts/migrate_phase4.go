...
func migrateInvoices(ctx context.Context, src *sql.DB, tx pgx.Tx, customerIDs, orderIDs IDMap) IDMap {
	t := time.Now()
	ids := make(IDMap)
	...
	for rows.Next() {
		...
		// Derive amount_paid from total - balance
		var amtPaid *float64
		...
		err := tx.QueryRow(ctx, `INSERT INTO invoices (legacy_id, invoice_number, active, customer_id, customer_name,
			...
			subtotal, tax, total_amount, amount_paid, balance, status, comments)
			...
			nd(subTotal), nd(taxAmt), nd(total), amtPaid, nd(balance), ns(status), ns(comments)).Scan(&newID)
		if err != nil {
	...
	logTable("invoices", srcCount, insCount, skipCount, time.Since(t))
	return ids
}

func migrateInvoiceDetails(ctx context.Context, src *sql.DB, tx pgx.Tx, invoiceIDs, orderIDs, vehicleIDs IDMap) {
	t := time.Now()
	...
	for rows.Next() {
		...
		// Convert qty decimal to int
		var qtyInt *int
		if qty.Valid && qty.Float64 != 0 {
			v := int(qty.Float64)
			qtyInt = &v
		}
		// Taxable is varchar(1) — "1" or "Y" means true
		taxBool := false
		if taxable.Valid {
			s := taxable.String
			taxBool = s == "1" || s == "Y" || s == "y"
		...
		_, err := tx.Exec(ctx, `INSERT INTO invoice_details (legacy_id, invoice_id, vehicle_id,
			...
			ns(desc), qtyInt, nd(unitPrice), nd(extended), taxBool, ns(item))
		if err != nil {
	...
	logTable("invoice_details", srcCount, insCount, skipCount, time.Since(t))
}

func migrateCreditMemos(ctx context.Context, src *sql.DB, tx pgx.Tx, customerIDs, invoiceIDs IDMap) {
	t := time.Now()
	if !tableExists(src, "A10") {
		logTable("credit_memos", 0, 0, 0, time.Since(t))
		return
	}
	// A10 is very minimal: A10Id, ToA00Id, FromA00Id, CreditDate, Amount
	...
	for rows.Next() {
		...
		// Generate credit_number since MSSQL has none
		...
		seen[creditNum] = true
		// ToA00Id is the invoice being credited
		_, err := tx.Exec(ctx, `INSERT INTO credit_memos (legacy_id, credit_number, invoice_id, credit_date, amount)
			VALUES ($1,$2,$3,$4,$5)
			ON CONFLICT (credit_number) DO NOTHING`,
			nint(oldID), creditNum, lookupFK(invoiceIDs, toA00id), nt(creditDate), nd(amount))
		if err != nil {
	...
	logTable("credit_memos", srcCount, insCount, skipCount, time.Since(t))
}

func migratePayments(ctx context.Context, src *sql.DB, tx pgx.Tx, customerIDs IDMap) IDMap {
	t := time.Now()
	ids := make(IDMap)
	...
	for rows.Next() {
		...
		// Derive applied_amount = amount - balance
	...
	logTable("payments", srcCount, insCount, skipCount, time.Since(t))
	return ids
}

func migratePaymentDetails(ctx context.Context, src *sql.DB, tx pgx.Tx, paymentIDs, invoiceIDs IDMap) {
	t := time.Now()
	...
	for rows.Next() {
		...
		_, err := tx.Exec(ctx, `INSERT INTO payment_details (legacy_id, payment_id, invoice_id, invoice_number, amount)
			VALUES ($1,$2,$3,$4,$5)`,
			nint(oldID), *payID, lookupFK(invoiceIDs, a00id), ns(invNum), nd(amount))
		if err != nil {
	...
	logTable("payment_details", srcCount, insCount, skipCount, time.Since(t))
}

func migrateDamageClaims(ctx context.Context, src *sql.DB, tx pgx.Tx, orderIDs, vehicleIDs, tripIDs IDMap) {
	t := time.Now()
	if !tableExists(src, "A40") {
		logTable("damage_claims", 0, 0, 0, time.Since(t))
		return
	}
	...
	for rows.Next() {
		...
		_, err := tx.Exec(ctx, `INSERT INTO damage_claims (legacy_id, claim_number, vehicle_id, vin,
			...
			nt(claimDate), nd(estAmt), nd(paidAmt), ns(status), ns(comments))
		if err != nil {
	...
	logTable("damage_claims", srcCount, insCount, skipCount, time.Since(t))
}

func migrateAccountsPayable(ctx context.Context, src *sql.DB, tx pgx.Tx, tripIDs, employeeIDs, truckIDs IDMap) {
	t := time.Now()
	if !tableExists(src, "A50") {
		logTable("accounts_payable", 0, 0, 0, time.Since(t))
		return
	}
	...
	for rows.Next() {
		...
		// Map active tinyint to status string
		var status *string
		if apType.Valid {
			s := nns(apType)
			if s != "" {
		...
		_, err := tx.Exec(ctx, `INSERT INTO accounts_payable (legacy_id, trip_id, vendor_name,
			payable_date, amount, status, description)
			VALUES ($1,$2,$3,$4,$5,$6,$7)`,
			nint(oldID), lookupFK(tripIDs, d20id), ns(vendorName),
			nt(txnDate), nd(amount), status, ns(desc))
		if err != nil {
	...
	logTable("accounts_payable", srcCount, insCount, skipCount, time.Since(t))
}
